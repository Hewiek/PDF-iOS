...
// 生成 dist 根目录 index.html，兼容部署环境将所有路由重写到 /index.html 的场景
try {
  const indexPath = path.join(distDir, 'index.html');
  let indexHtml = template.replace('{{TITLE}}', 'Page: pdf-converter-ios');
  indexHtml = indexHtml.replace('{{ENTRY}}', './pages/pdf-converter-ios.js');
  indexHtml = indexHtml.replace('{{BOOTSTRAP_PATH}}', './assets/html-template-bootstrap.js');
  fs.writeFileSync(indexPath, indexHtml, 'utf8');
  console.log('生成: index.html');
  generatedCount++;
} catch (e) {
  console.warn('⚠ 生成 index.html 失败:', e && e.message ? e.message : e);
}
...
if (fs.existsSync(srcBootstrap)) {
  ...
  if (fs.existsSync(srcChunksDir)) {
    ...
    for (const file of chunkFiles) {
      ...
      fs.copyFileSync(src, dest);
      console.log(`✓ chunks/${file} 已复制到 dist/assets/chunks`);
    }
  }
} else {
